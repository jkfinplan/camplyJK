...
class CamavaProvider(BaseProvider):
    ...
    def _extract_site_info(
        ...
        try:
            ...
            # First check for cabins (which are string values, not numeric)
            cabin_match = re.search(r'Cabin\s+([A-Z0-9]+)', text, re.IGNORECASE)
            if cabin_match:
                cabin_name = cabin_match.group(1)
                site_name = f"Cabin {cabin_name}"
                site_number = cabin_name  # Use the cabin name/identifier as-is
            else:
                # Check for regular sites with numeric identifiers
                ...
                if site_match:
                    ...
                    # Fallback: try to find any site identifier pattern
                    # This handles cases like "Site A", "Site B", etc.
                    fallback_match = re.search(r'Site\s+([A-Z0-9]+)', text, re.IGNORECASE)
                    if fallback_match:
                        site_name = f"Site {fallback_match.group(1)}"
                        site_number = fallback_match.group(1)
                    else:
            ...
            # Extract site type
            site_type = "Standard"
            if 'cabin' in text.lower():
                site_type = "Cabin"
            elif 'RV' in text or 'rv' in text.lower():
                site_type = "RV"
